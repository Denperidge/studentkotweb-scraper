...
var searchPage, pageIndex;

async function ParseSearchPage(index) {
	console.log("Getting all rooms from page " + index);

	[searchPage, pageIndex] = await GetElementsFromUrl(searchUrl + "&page=" + index, ["div.m-teaser a.a-button-secondary", ".m-pagination-item:last-child"], all=true);
	
	searchPage.forEach(async (roomLink) => {
